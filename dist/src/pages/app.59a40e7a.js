...
const getWeatherData = async (lat, long)=>{
    try {
        const res = await fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${long}&appid=0ac8d24292bec7ba2bf644e405cffefb`);
        const data = await res.json();
        temperature.textContent = `${(data.main.temp - 273.15).toFixed(1)} ℃`;
        console.log(data);
        sunrise.textContent = `Sunrise: ${new Date(data.sys.sunrise * 1000).toLocaleTimeString("default")}`;
        sunset.textContent = `Sunset: ${new Date(data.sys.sunset * 1000).toLocaleTimeString("default")}`;
...
},{}]},["7ZoMj","8lRBv"], "8lRBv", "parcelRequire7dd5")
